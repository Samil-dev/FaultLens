...
**Duration is a real parameter, not a cosmetic one.** The configured
duration (`app/chaos/duration_model.py`) deterministically affects the
outcome: a brief experiment (≤15s) is contained to direct dependents only,
while a longer one cascades through the full dependency chain; recovery
time and metric severity both scale with how long the failure ran; and a
sustained experiment (≥60s) can leave a node more than one hop from the
target unable to fully recover at all — the same experiment run twice at
the same duration always produces the same result, but 10s/30s/60s produce
three genuinely different ones. See `Frontend/e2e/duration-effects.spec.ts`
for a live UI-driven proof.
...
Every number and status FaultLens shows falls into exactly one of these —
never mixed, never relabeled to look more (or less) authoritative than it is:

| Category | Meaning | Where |
|---|---|---|
| **REAL** | Genuine computed output of the simulation model, driven by the imported topology and the configured experiment parameters (including duration). | Propagation path, affected nodes, blast radius, recovery time/status, resilience score, risk level. |
| **ESTIMATED** | A client-side preview computed *before* running anything — explicitly labeled "Estimated" in the UI, never presented as a measured result. | The Digital Twin's pre-experiment "Impact Preview" / criticality. |
| **MOCK** | A deterministic, offline stand-in used for development/demo, always labeled with its own name — never presented as IBM Bob. | `MockAIProvider` (`provider: "mock"` in every `AIInsight`). |
| **NOT_CONFIGURED** | A real capability whose connection is missing required external configuration — reported honestly, never faked. | `BobAIProvider` without `BOB_API_ENDPOINT`/`BOB_API_KEY`; `AIInsight.status: "not_configured"`. |
| **SIMULATED (never real infrastructure)** | Applies to the whole Chaos Engineering layer — no real service, container, or network is ever touched. | Every experiment type, always. |
...
- Deterministic before/after metrics simulation, with duration a real
  parameter — not cosmetic — affecting propagation depth, recovery time,
  metric severity, and whether a distant node fails to recover at all
  (`app/chaos/duration_model.py`)
...
- 279 backend tests; a 7-spec Playwright suite (Core Workflow, persistence &
  switching, hardening: system-switch isolation + import validation, IBM Bob
  / MCP integration via a real subprocess round-trip, and duration effects
  proven live through the real experiment UI) driving the real UI against
  the real backend; CI running backend tests + frontend build
...
**Backend:** 279 tests, all passing, using `pytest` + FastAPI's `TestClient`
...
complete system/experiment/comparison API surface, duration-model behavior
(unit-, engine-, and full-HTTP-level — `test_duration_model.py` plus
duration-specific tests in `test_chaos_engine.py`/`test_experiment_api.py`),
and a real MCP protocol round-trip (`test_mcp_protocol_roundtrip.py`) that
spawns `app.mcp.server` as a subprocess and drives it over the actual stdio
...
import validation error surfacing), the IBM Bob / MCP integration
...
indicator reflects that genuine cross-process activity — and duration
effects (`duration-effects.spec.ts`), which fills the real experiment
duration input at 10s/30s/60s and confirms the Resilience Panel shows three
different scores for the identical experiment.
