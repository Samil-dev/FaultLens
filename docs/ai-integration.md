...
| State | Meaning |
|---|---|
| `Checking…` | Initial load, before the first poll resolves. |
| `MCP unavailable` | `app.mcp.server` failed to import in this backend, or the status request itself failed. |
| `Not connected` | The MCP server is available, but no MCP tool has ever been called against this database. |
| `Active via MCP` | A real MCP tool call was recorded within the last 2 minutes. |
| `MCP available` | The MCP server is available and has been used before, but not recently. |

`Active via MCP` and `MCP available` both show which tool was called and
...
Watch the header: "IBM BOB · Not connected" becomes
"IBM BOB · Active via MCP" within ~10 seconds (the poll interval) — genuine
